...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item You can also drag and drop a \texttt{.geodraw}/\texttt{.json} file onto the canvas to open it.
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \texttt{Shift + ArrowUp}/\texttt{Shift + ArrowDown} (Object Browser): extend linked selection to previous/next object of the same type
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \gdicon{screenshots/tools/export_clip_rect.png} \textbf{Crop Area -- Box}: create a rectangular crop area for export.
  \item \gdicon{screenshots/tools/export_clip.png} \textbf{Crop Area -- Freeform}: create a polygon crop area for export (close by clicking near the first vertex).
\end{itemize}
These two tools are also reachable directly from the \textbf{Crop area} block in the Export Tab section below -- that panel is the main way to start a crop, since it walks you through the whole process without needing to hunt for the tool in the palette.
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{Invert}: invert a source line/circle with respect to a selected inversion circle.
\end{itemize}
All transform tools preserve source styling on the transformed image (stroke/fill/marks/arrows/labels).
...
\begin{itemize}[leftmargin=1.5em]
  \item Tabs: All, Points, Lines, Shapes, Angles, Labels, Numbers.
  \item Auto-follow: selected object switches to its tab automatically.
  \item Pin button (lock icon): freeze current tab and disable auto-follow until unpinned.
  \item Click row to select object.
  \item Link button (chain icon): link/unlink object for batch style/delete operations.
  \item Linking is type-restricted: linked objects must be the same type (point with point, line with line, etc.).
  \item \texttt{Shift + ArrowUp}/\texttt{Shift + ArrowDown}: add previous/next same-type row to linked selection.
...
\subsection{Linked object editing}
\begin{itemize}[leftmargin=1.5em]
  \item Link multiple same-type objects from Object Browser.
  \item For linked objects, style/field edits in the Properties panel apply to all linked objects of that type.
  \item The delete action becomes \textbf{Delete All} and removes the linked selection.
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{Convert to Segment} (line only): hide source \texttt{Line(A,B)} and create \texttt{Segment(A,B)} with matching style/label fields.
  \item If multiple lines are linked, conversion applies to all linked convertible lines.
\end{itemize}
...
Special tools (Fixed Angle, Fixed Radius Circle, Crop Area, Copy Style) show context-specific instructions and inputs.
...
\begin{itemize}[leftmargin=1.5em]
  \item \textbf{Point/transform}: \texttt{Point(x,y)}, \texttt{Midpoint(A,B)}, \texttt{Midpoint(s)}, \texttt{Incenter(A,B,C)}, \texttt{Orthocenter(A,B,C)} (\texttt{Ortho}), \texttt{Centroid(A,B,C)}, \texttt{Circumcenter(A,B,C)}, \texttt{Translate(P,A,B)}, \texttt{Rotate(P,O,expr[,CW|CCW])}, \texttt{Dilate(P,O,k)}, \texttt{Reflect(P,l|s|O)}.
  ...
  \item \textbf{Angle/polygon}: \texttt{Angle(A,B,C)} for degree value, \texttt{MarkedAngle(A,B,C)} for an angle object, \texttt{AngleFixed(V,A,expr[,CW|CCW])}, \texttt{Sector(O,A,B)}, \texttt{Polygon(A,B,C,...)}, \texttt{RegularPolygon(A,B,n[,CW|CCW])}.
  ...
  \item \textbf{Numeric query}: \texttt{Distance(...)}, \texttt{Area(...)}, \texttt{Perimeter(...)}, \texttt{Inradius(A,B,C)}, \texttt{Circumradius(A,B,C)} return scalar values (no object created).
\end{itemize}
...
\section{Export Tab}
\subsection{Crop area}
The \textbf{Crop area} block is the main way to export only part of the figure. When no crop area exists yet, it shows two buttons:
\begin{itemize}[leftmargin=1.5em]
  \item \textbf{Box}: draw a rectangular crop area (two opposite corners).
  \item \textbf{Freeform}: draw a polygon crop area (click each corner, then click near the first vertex to close it).
\end{itemize}
Clicking either button arms the matching tool and shows an inline step hint (``Now click...'') in the panel so you don't need to leave the Export tab to see what to do next. Once an area is drawn, \textbf{Export only the box/shape I drew} is checked automatically, and the block switches to three pills:
\begin{itemize}[leftmargin=1.5em]
  \item \textbf{Done}: stop drawing and return to the Move tool.
  \item \textbf{Redraw}: re-arm the same tool to draw the area again.
  \item \textbf{Clear}: remove the crop area and go back to exporting the whole figure.
\end{itemize}
While an area exists, drag its square handles on the canvas to adjust it, or uncheck \textbf{Export only the box/shape I drew} to temporarily export the full figure without discarding the area you drew.

The same Box and Freeform crops can also be started from the \textbf{Crop Area -- Box} / \textbf{Crop Area -- Freeform} tools in the Move group of the Tool Palette (Section~\ref{sec:tool-palette}); using the panel is recommended since it keeps the step hints and Done/Redraw/Clear controls in one place.

\subsection{TikZ export options}
\begin{itemize}[leftmargin=1.5em]
  \item \textbf{Use Current View}: export based on current camera viewport.
  \item \textbf{Export only the box/shape I drew}: apply the crop area from the Crop area block above.
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item Right-click inside PDF preview to save snapshot as \textbf{PDF}, \textbf{SVG}, or \textbf{PNG}.
...
\subsection{Crop export does not apply}
If you used the Freeform crop, ensure you closed the polygon (click near the first point). Then check that \textbf{Export only the box/shape I drew} is enabled in the Crop area block of the Export tab -- drawing an area normally enables it automatically, but it can be unchecked afterward.
...
\subsection*{A.6 Export Panel + Crop Area}
% \begin{figure}[h]
% \centering
% \includegraphics[width=\linewidth]{screenshots/export-panel.png}
% \caption{Export options and Crop area workflow.}
% \end{figure}
