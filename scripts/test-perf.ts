...
console.log(`Benchmarking scene with ${scene.points.length} points...`);

const WARMUP_TICKS = 100;
const TEST_TICKS = 500;
...
for (let i = 0; i < TEST_TICKS; i++) {
    ...
    if (stats) totalCalls += stats.totalNodeEvalCalls;
}

const end = performance.now();
const duration = end - start;
const avgMs = duration / TEST_TICKS;

console.log(`Results:`);
console.log(`  Total Time: ${duration.toFixed(2)}ms`);
console.log(`  Ticks: ${TEST_TICKS}`);
console.log(`  Avg Time per Tick: ${avgMs.toFixed(4)}ms`);
console.log(`  Total Node Evals: ${totalCalls}`);
console.log(`  Avg Node Evals per Tick: ${(totalCalls / TEST_TICKS).toFixed(2)}`);
