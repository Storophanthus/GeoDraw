...
export function drawPolygons(
  ...
  for (const polygon of scene.polygons) {
    drawPolygonObject(ctx, scene, polygon.id, camera, vp, selectedObject, recentCreatedObject, copySource, ownedEdgePresence);
  }
  ctx.restore();
}

export function drawPolygonObject(
  ctx: CanvasRenderingContext2D,
  scene: SceneModel,
  polygonId: string,
  camera: Camera,
  ...
  recentCreatedObject: DrawableObjectSelection,
  copySource: DrawableObjectSelection,
  ownedEdgePresence?: Set<string>
): void {
  const polygon = scene.polygons.find((item) => item.id === polygonId);
  ...
  if (worldPoints.some((world) => !world)) return;
  const screenPoints = worldPoints.map((world) => camMath.worldToScreen(world!, camera, vp));

  const effectiveOwnedEdgePresence = ownedEdgePresence ?? new Set<string>();
  ...
  for (let i = 0; i < polygon.pointIds.length; i += 1) {
    ...
    if (effectiveOwnedEdgePresence.has(scopedKey)) continue;
    ctx.beginPath();
