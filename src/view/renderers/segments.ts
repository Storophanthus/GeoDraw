...
export function drawSegments(
  ...
  for (const seg of scene.segments) {
    drawSegmentObject(ctx, scene, seg.id, camera, vp, selectedObject, recentCreatedObject, copySource);
  }

  ctx.restore();
}

export function drawSegmentObject(
  ctx: CanvasRenderingContext2D,
  scene: SceneModel,
  segmentId: string,
  camera: Camera,
  ...
  const seg = scene.segments.find((item) => item.id === segmentId);
