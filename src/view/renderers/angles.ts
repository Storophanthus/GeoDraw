...
export function drawAngles(
  ...
  for (const entry of resolvedAngles) {
    drawResolvedAngleObject(ctx, entry, camera, vp, selectedObject, recentCreatedObject, mapStrokeWidth);
  }
}

export function drawAngleObject(
  ctx: CanvasRenderingContext2D,
  resolvedAngles: ResolvedAngleForRender[],
  angleId: string,
  camera: Camera,
  ...
  const entry = resolvedAngles.find((item) => item.angle.id === angleId);
  if (!entry) return;
  drawResolvedAngleObject(ctx, entry, camera, vp, selectedObject, recentCreatedObject, mapStrokeWidth);
}

function drawResolvedAngleObject(
  ctx: CanvasRenderingContext2D,
  entry: ResolvedAngleForRender,
  camera: Camera,
  vp: Viewport,
  selectedObject: DrawableObjectSelection,
  recentCreatedObject: DrawableObjectSelection,
